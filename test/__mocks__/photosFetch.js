...
exports.getPhotosFetch = url => {
  const { query } = urlParser.parse(url, true);
  return Promise.resolve(JSON.stringify(exports.albumPhotos[query.albumId]));
};

exports.getAlbumsFetch = url => {
  const { path } = urlParser.parse(url, true);
  const elements = path.split('/');
  const index = Number(elements.pop());
  if (index === 'NaN') {
    return Promise.resolve(exports.albums);
  }
  return Promise.resolve(exports.albums[index - 1]);
};
